...
- IPC 树彻底移除 HTTP carrier：`startHost` 禁用 `webserver` 行（以及依赖 webserver 绑定主机的 `directory-picker` auto 行），`ClientModuleRegistry` 让 `webServer` 可选并暴露 `serveBundleFetch`（与 HTTP 载体包装的是同一个 fetch 形状处理器，由 `dsh://` 协议驱动），`dsh-web-app`、`dsh-client-connection`、`dsh-client-hmr` 全部容忍缺失 `webServer`。无 key 冒烟 boot 验证：`webServer` 缺席、`clientModules` 挂载且含完整图、bundle fetch 对未知 id 返回 404。
- IPC 树在 auto 行位置钉住 **Electron 对话框后端**（`apps/desktop/src/main/directory-picker.ts`）：`DirectoryPicker` 子类，其 `native` 能力打开 `dialog.showOpenDialog`（electron 惰性解析，纯 Node 测试环境可加载模块），配 native client surface。koffi/osascript/zenity 后端仍服务 `dsh web`。
- **打包**以 `pnpm package` / `pnpm package:win` 交付：`scripts/stage-app.mjs` 把依赖树 deploy 到 pnpm workspace 之外的临时目录（`pnpm deploy --legacy`），展平指向仓库的 workspace junction（pnpm 对 `link:` overrides 如 cosmokit/schemastery 保留 junction），把 `workspace:^` 版本改写为具体版本，并复制构建好的 `lib/`；`scripts/package-app.mjs` 随后对该 staging 树运行 electron-builder。Windows 无提权 shell 无法提取 winCodeSign 的符号链接，故 `package:win` 禁用签名/编辑（`signAndEditExecutable=false`、dir 目标）；验证产物为解包后的 `DeepSeek Harness.exe` 与约 4600 条目的 asar（main/preload/protocol 入口、`dsh-web-frontend/dist`、完整依赖闭包，`node-pty`/`koffi` 从 asar 解包）。
- 已记录的 IPC 缺口（渲染器 AbortSignal 不跨 IPC、`/api/session.export` 未在 IPC 上分块）已由 [electron-ipc-abort-and-chunked-api-routing](2026-08-18-electron-ipc-abort-and-chunked-api-routing.md) 闭合：中止经 request-id 的 `abort` 桥通道传递，`/api/*` 经 `dsh://` 协议流式交给 `toFetchHandler`。
...
`pnpm run test:gui` 全绿。新增 client specs：`tests/electron-api-client.client.spec.ts`（桥 unary/respond/流/abort/坏帧/stream-end、doFetch 字段序列化）、`tests/ipc-rpc.client.spec.ts`（关联、mismatch、传输失败、目标围栏、未序列化 signal）、`tests/client-apply.client.spec.ts` 的 IPC 场景（传输事实 + 桥选择 `ElectronApiClient`、部分事实保持 `WebApiClient`、fixture 优先）。connection 包覆盖率：`src/` 全文件 statements/branches/functions/lines 100%。`client-modules` 新增 `serveBundleFetch` specs（无 `webServer` 的 fetch-only 构造、404/405/400 分支）。`web-app` 新增无 `webServer` spec（无 dist 服务、无 URL、`webRuntime` 仍提供）。Electron 对话框后端有 `apps/desktop/tests/directory-picker.spec.ts`（选定路径、取消、空选择、对话框错误、预中止 signal；electron 模块被 mock）。无 HTTP carrier 的 IPC 树由无 key 的 `startHost({ webServerRequired: false })` boot 冒烟验证：断言无 `webServer` 服务、Electron `directoryPicker` 挂载且带 `native` 能力、`clientModules` 挂载且含完整图、fetch 处理器正常应答。
