...
- The IPC-known gaps remain as documented: renderer AbortSignal does not cross IPC (unary cancellation degrades to main-side completion) and `/api/session.export` is not chunked over IPC.
