...
- The IPC-known gaps remain as documented: renderer AbortSignal does not cross IPC (unary cancellation degrades to main-side completion), `/api/session.export` is not chunked over IPC, and loopback mode keeps the HTTP server alive.
